...
import net.degoes.zio.SequentialCause.composed
import zio._
...
object ErrorRecoveryOrElse extends ZIOAppDefault {
   ...
   */
  val run =
    failed.orElse(ZIO.succeed("Nevermind"))
}
...
object ErrorShortCircuit extends ZIOAppDefault {
   ...
   */
  val run =
    failed.orElse(ZIO.succeed("Nevermind").exitCode)
}
...
object ZIOFinally extends ZIOAppDefault {
  ...
  val run = tickingBomb2.fork.map(_.interrupt)
}
...
object Sandbox extends ZIOAppDefault {
   ...
   */
  val run =
    composed.sandbox.tapError(c => Console.printLine(c.prettyPrint))
}
